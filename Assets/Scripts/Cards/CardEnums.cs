...
static class CardEnums
{
    ...
    public static SortedSet<T2> GetValidFlags<T1, T2>(T1 key)
    {
        if (FlagSets.ContainsKey((typeof(T1), typeof(T2))))
        {
            Dictionary<T1, SortedSet<T2>> dict = FlagSets[(typeof(T1), typeof(T2))];
            if (!dict.ContainsKey(key))
            {
                dict[key] = new SortedSet<T2>();
            }
            return dict[key];
        }
        Debug.Assert(false, "Flags are undefined for <" + typeof(T1).ToString() + "," + typeof(T2).ToString() + ">");
        return new SortedSet<T2>();
    }
    ...
    static CardEnums()
    {
        ...
        RemoveFlags(EffectType.DEAL_DAMAGE, new TriggerCondition[] { TriggerCondition.IS_ALIVE });
        ...
        RegisterFlags(TargetType.PLAYERS, new TargettingType[] { TargettingType.SELF, TargettingType.FOE, TargettingType.ALL, TargettingType.TARGET, TargettingType.UP_TO_TARGET });
        ...
        RegisterFlags(CardType.CREATURE, new TriggerCondition[] { TriggerCondition.ON_SELF_ENTER, TriggerCondition.ON_CREATURE_ENTER, TriggerCondition.ON_SELF_DIES,
            TriggerCondition.ON_CREATURE_DIES, TriggerCondition.ON_SELF_DAMAGE_TAKEN, TriggerCondition.ON_SELF_DAMAGE_DEALT, TriggerCondition.ON_STACK_UPDATED, TriggerCondition.IS_ALIVE});
